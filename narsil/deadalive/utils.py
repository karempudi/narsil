...
class createTrainingData(object):
    ...
    def __init__(self, segmentedDir=None, phaseDir = None, fileformat='.tiff', saveName='states.npy'):
        ...
        # There are 4 states, Moving, NotMoving, PartlyDead, AllDead, NoCells, CellsVanished
        ...
        # Radio Buttoms to set states like blobs disappear, blobs move.
        self.states = ['Moving', 'NotMoving','PartlyDead', 'AllDead', 'NoCells', 'CellsVanished']
        self.checkax = plt.axes([0.025, 0.6, 0.15, 0.15], facecolor=self.axcolor)
    ...
    def __getitem__(self, idx):
        ...
        return {'segImage': io.imread(segFileName), 'phaseImage': io.imread(phaseFileName)}
    ...
    def updateFig(self, val):
        ...
        if index in self.indices:
            ...
            #self.fig.canvas.draw_handle()
    def applyStates(self, label):
        row_index = self.states.index(label)
        #print(f"{row_index} --- {self.currentTimePoint} -- {self.checkButtons.get_status()[row_index]}")
    ...
    def save(self, save):
        ...
        with open(self.savedFileName, 'wb') as f:
            ...
            print(f'{self.savedFileName} saved :)')
